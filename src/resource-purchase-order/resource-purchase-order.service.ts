...
export class ResourcePurchaseOrderService {
  ...
  async create(createResourcePurchaseOrderDto: CreateResourcePurchaseOrderDto) {
    ...
    const newResourcePurchaseOrder =
      await this.prisma.resourcePurchaseOrder.create({
    ...
    if (!newResourcePurchaseOrder) {
      ...
      throw new BadRequestException(
        'No se pudo crear el recurso de la orden de compra.',
      );
    }

    this.logger.log(
      `Resource purchase order created successfully with id: ${newResourcePurchaseOrder.purchaseOrderId}`,
    );
    return {
  ...
  async findAllByPurchaseOrderId(purchaseOrderId: number) {
    this.logger.log(
      `Fetching all resource purchase orders for purchase order id: ${purchaseOrderId}`,
    );

    this.logger.log(
      `Checking if purchase order exists with id: ${purchaseOrderId}`,
    );
    ...
    if (!exists) {
      this.logger.warn(
        `Purchase order does not exist with id: ${purchaseOrderId}`,
      );
      throw new BadRequestException('La orden de compra no existe.');
    }

    const resourcePurchaseOrders =
      await this.prisma.resourcePurchaseOrder.findMany({
        where: { purchaseOrderId },
    ...
    if (!resourcePurchaseOrders) {
      ...
      throw new BadRequestException(
        'No se pudieron obtener los recursos de la orden de compra.',
      );
    }

    this.logger.log(
      `Fetched ${resourcePurchaseOrders.length} resource purchase orders`,
    );
    return {
  ...
  async findOne(resourcePurchaseOrderId: number) {
    this.logger.log(
      `Fetching resource purchase order with id: ${resourcePurchaseOrderId}`,
    );
    const resourcePurchaseOrder =
      await this.prisma.resourcePurchaseOrder.findUnique({
    ...
    if (!resourcePurchaseOrder) {
      this.logger.warn(
        `Resource purchase order does not exist with id: ${resourcePurchaseOrderId}`,
      );
      throw new BadRequestException('La orden de compra no existe.');
    }
  ...
  async update(
    id: number,
    updateResourcePurchaseOrderDto: UpdateResourcePurchaseOrderDto,
  ) {
    this.logger.log(`Updating resource purchase order with id: ${id}`);
    const updatedResourcePurchaseOrder =
      await this.prisma.resourcePurchaseOrder.update({
    ...
    if (!updatedResourcePurchaseOrder) {
      this.logger.error(
        `Failed to update resource purchase order with id: ${id}`,
      );
      throw new BadRequestException(
        'No se pudo actualizar el recurso de la orden de compra.',
      );
    }

    this.logger.log(
      `Resource purchase order with id: ${id} updated successfully`,
    );
    return {
      statusCode: HttpStatus.OK,
  ...
  async remove(resourcePurchaseOrderId: number) {
    this.logger.log(
      `Deleting resource purchase order with id: ${resourcePurchaseOrderId}`,
    );

    await this.findOne(resourcePurchaseOrderId);

    const deletedResourcePurchaseOrder =
      await this.prisma.resourcePurchaseOrder.delete({
        where: { resourcePurchaseOrderId },
      });

    if (!deletedResourcePurchaseOrder) {
      this.logger.error(
        `Failed to delete resource purchase order with id: ${resourcePurchaseOrderId}`,
      );
      throw new BadRequestException(
        'No se pudo eliminar el recurso de la orden de compra.',
      );
    }

    this.logger.log(
      `Resource purchase order with id: ${resourcePurchaseOrderId} deleted successfully`,
    );
    return {
      statusCode: HttpStatus.OK,
