...
export class ResourcePurchaseOrderController {
  ...
  create(@Body() createResourcePurchaseOrderDto: CreateResourcePurchaseOrderDto) {
    this.logger.log(`Creating resource purchase order: ${JSON.stringify(createResourcePurchaseOrderDto)}`);
  ...
  findAllByPurchaseOrderId(@Param('id', ParseIntPipe) id: number) {
    this.logger.log(`Finding all resource purchase orders for Purchase Order ID: ${id}`);
  ...
  @Patch(':id')
  update(@Param('id', ParseIntPipe) id: number, @Body() updateResourcePurchaseOrderDto: UpdateResourcePurchaseOrderDto) {
