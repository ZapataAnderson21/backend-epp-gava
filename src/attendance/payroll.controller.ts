// payroll.controller.ts  (Planillas EN GENERAL)
...
@Controller('payroll')
export class GlobalPayrollController {
  constructor(private readonly attendanceService: AttendanceService) {}

  // Tabla general (todas las semanas con asistencias)
  @Get('summary')
  async globalSummary(
    @Query('asOfDate') asOfDate?: string,
    @Query('formatCurrency') formatCurrencyFlag?: string,
  ) {
    const rows = await this.attendanceService.getGlobalWeeklyPayrollSummary(
  ...
  // Detalle global de una semana (wireframe “Click en el See button de planillas general”)
  @Get('weeks/:weekId')
  async globalDetail(
    ...
    const money = String(formatCurrencyFlag) === 'true';
    const detail = await this.attendanceService.getGlobalWeeklyPayrollForWeek(
      weekId,
      asOfDate ? new Date(asOfDate) : new Date(),
    );
