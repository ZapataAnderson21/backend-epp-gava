...
export class RequestWorkerService {
  ...
  async create(createRequestWorkerDto: CreateRequestWorkerDto) {
    ...
    if (!newRequestWorker) {
      this.logger.error(
        'Failed to create request worker',
        createRequestWorkerDto,
      );
      throw new BadRequestException(
        'No se pudo crear la solicitud para un trabajador específico.',
      );
    }

    this.logger.log(
      'Request worker created successfully',
      JSON.stringify(newRequestWorker),
    );
    return {
      statusCode: HttpStatus.CREATED,
      message:
        'La solicitud para un trabajador específico ha sido creada exitosamente.',
  ...
  async findAllByRequestId(requestId: number) {
    ...
    this.logger.log(
      `Found ${requestWorkers.length} request workers for request ID: ${requestId}`,
    );
    return {
      statusCode: HttpStatus.OK,
      message:
        'Solicitudes para trabajadores específicos encontradas exitosamente.',
      data: requestWorkers,
    };
  }
  ...
  async findOne(requestWorkerId: number) {
    ...
    this.logger.log(
      `Request worker with ID ${requestWorkerId} found`,
      JSON.stringify(requestWorker),
    );
    return {
      statusCode: HttpStatus.OK,
      message:
        'Solicitud para un trabajador específico encontrada exitosamente.',
      data: requestWorker,
    };
  }
  ...
  async update(
    ...
    if (!updatedRequestWorker) {
      this.logger.error(
        `Failed to update request worker with ID: ${requestWorkerId}`,
        JSON.stringify(updateRequestWorkerDto),
      );
      throw new BadRequestException(
        'No se pudo actualizar la solicitud para un trabajador específico.',
      );
    }

    this.logger.log(
      `Request worker with ID ${requestWorkerId} updated successfully`,
      JSON.stringify(updatedRequestWorker),
    );
    return {
      statusCode: HttpStatus.OK,
      message:
        'La solicitud para un trabajador específico ha sido actualizada exitosamente.',
  ...
  async remove(requestWorkerId: number) {
    ...
    if (!deletedRequestWorker) {
      this.logger.error(
        `Failed to delete request worker with ID: ${requestWorkerId}`,
      );
      throw new BadRequestException(
        'No se pudo eliminar la solicitud para un trabajador específico.',
      );
    }

    this.logger.log(
      `Request worker with ID ${requestWorkerId} deleted successfully`,
      JSON.stringify(deletedRequestWorker),
    );
    return {
      statusCode: HttpStatus.OK,
      message:
        'La solicitud para un trabajador específico ha sido eliminada exitosamente.',
  ...
  async requestExistsAndIsDraft(requestId: number) {
    ...
    if (request.status !== RequestStatus.draft) {
      this.logger.error(
        `The operation cannot be performed because the request with ID ${requestId} is not in draft status`,
      );
      throw new BadRequestException(
        'La operación no puede ser realizada porque la solicitud no está en estado de borrador',
      );
    }

    this.logger.log(
      `Request with ID ${requestId} exists and is in draft status`,
    );
    return {
      statusCode: HttpStatus.OK,
      message:
  ...
  async verifyWorkerExists(workerId: number) {
    ...
    if (!worker) {
      ...
      throw new NotFoundException(
        `El usuario con ID ${workerId} no fue encontrado.`,
      );
    }
