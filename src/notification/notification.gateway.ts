  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleConnection(client: Socket) {
    ...
    try {
      ...
      if (!userId) {
        this.logger.warn(`Client ${client.id} connection rejected: Invalid token payload (no userId)`);
  ...
  handleMarkAsRead(client: Socket, payload: { notificationId: number }) {
    const userId = client.data.userId;
    this.logger.log(`User ${userId} marking notification ${payload.notificationId} as read`);
